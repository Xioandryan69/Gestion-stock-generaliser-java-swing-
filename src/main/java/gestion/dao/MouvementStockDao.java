...
public class MouvementStockDao {
    ...
    /**
     * Récupère les mouvements validés pour un produit jusqu'à (et incluant) la date fournie.
     */
    public List<MouvementStock> findByProduitIdUntilDate(int idProduit, java.time.LocalDate date) throws Exception {
        List<MouvementStock> mouvements = new ArrayList<>();
        String sql = "SELECT * FROM mouvement_stock WHERE id_produit = ? AND statut = 'VALIDÉ' " +
                "AND date_mouvement <= ? ORDER BY date_mouvement ASC, id ASC";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            stmt.setInt(1, idProduit);
    ...
    /**
     * Récupère tous les mouvements validés jusqu'à (et incluant) la date fournie.
     */
    public List<MouvementStock> findAllUntilDate(java.time.LocalDate date) throws Exception {
        List<MouvementStock> mouvements = new ArrayList<>();
        String sql = "SELECT * FROM mouvement_stock WHERE statut = 'VALIDÉ' AND date_mouvement <= ? ORDER BY date_mouvement ASC, id ASC";
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
