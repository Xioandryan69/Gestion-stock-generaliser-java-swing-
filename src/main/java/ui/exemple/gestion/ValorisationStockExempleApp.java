...
public class ValorisationStockExempleApp extends JFrame {
    ...
    private StockSnapshot computeSnapshot(int idProduit, LocalDate date) throws Exception {
        ...
        try (Connection connection = DatabaseConfig.getConnection();
             PreparedStatement stmt = connection.prepareStatement(
                     "SELECT type_mouvement, quantite, prix_unitaire " +
                             "FROM mouvement_stock WHERE id_produit = ? AND statut = 'VALIDÉ' AND date_mouvement <= ? " +
                             "ORDER BY date_mouvement ASC, id ASC")) {
            stmt.setInt(1, idProduit);
            stmt.setDate(2, java.sql.Date.valueOf(date));
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    BigDecimal qty = rs.getBigDecimal("quantite");
                    BigDecimal pu = rs.getBigDecimal("prix_unitaire");
                    if ("ENTREE".equalsIgnoreCase(rs.getString("type_mouvement"))) {
                        quantite = quantite.add(qty);
                        valeur = valeur.add(qty.multiply(pu));
                    } else {
                        quantite = quantite.subtract(qty);
                        valeur = valeur.subtract(qty.multiply(pu));
                    }
                }
            }
        }
        ...
        BigDecimal cump = quantite.compareTo(BigDecimal.ZERO) == 0
                ? BigDecimal.ZERO
                : valeur.divide(quantite, 4, RoundingMode.HALF_UP);
        return new StockSnapshot(quantite, valeur, cump);
    }
